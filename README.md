...
- make browsing much easier with the help of AdvCiv-SAS' sevopedia (search bar (with special characters  support (e.g., `[`, `_`, `(`)), keyboard navigation (UP/DOWN for category-list items; LEFT/RIGHT for BACK/NEXT pages visited), leader animation attitude (furious, annoyed, cautious, pleased, friendly) and action (noanim, greeting, agree, disagree) buttons)
